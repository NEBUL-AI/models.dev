...
- Whole-catalog faults fail closed: an empty response, or one where nothing matches the chat-model filter (`model_type: "llm"`, `mode: "chat"`), throws in `parseModels` before any file is written or deleted — mirroring the ingest-side guards. Partial faults fail closed too: a catalog that parses but shows fewer usable chat models than `MIN_CHAT_MODELS` (seeded at half the known-good fleet size, raised deliberately as coverage grows) is treated as structurally incomplete and throws, so a per-lab serving outage or truncated payload cannot prune healthy local files.
...
- `reasoning_options`: authored controls always win (they are probe-verified evidence, while the advertised `reasoning_efforts` list can be wrong — on 2026-09-23 it advertised `low|medium|high|max` for `Mistral-Medium-3.5`, which the served engine rejects for every value but `high`). A new entry with nothing authored falls back to the advertised list. A reasoner with neither advertised efforts nor authored controls fails sync for manual authoring rather than writing an empty control set.
- An authored `reasoning = false` is the escape hatch for a served ID whose lab model reasons but which this host runs with thinking disabled (catalog reports `supports_reasoning = false` and no `reasoning_efforts`). The override is kept, the missing-reasoning-options guard does not fire, and any authored `reasoning_options`/`interleaved` are dropped from the synced file.
- Evidence rule for Nebul reasoning claims (relay host): never author a reasoning claim — an effort set, `[]`, or `reasoning = false` — from catalog absence alone. `supports_reasoning = false` lags server-side enablement often (probed 2026-09-23: Kimi-K3, Qwen3.8, Qwen3.5 and all three Nemotron models reason despite the flag, as does `openai/gpt-oss-120b` per its docs). Each claim needs dated per-ID evidence in the leading header: a live chat-completions probe (per-effort acceptance plus trace-field observation with a date, as in the ai& provider's headers) or, for preview-gated IDs the probe key cannot serve, dated catalog plus sibling evidence.
- `interleaved`, `status`, and other fields the endpoint does not expose are preserved from existing files; the reasoning trace channel (`reasoning_content` vs `message.reasoning` vs inline `<thinking>`) varies per served engine, not per family — probes on 2026-09-23 found `message.reasoning` for Kimi-K3 and Mistral-Medium-3.5 (`interleaved = true`) but `reasoning_content` for the GLM/Qwen/Nemotron IDs, so each ID's channel needs probe evidence too.
- `BASE_MODEL_ALIASES` in the module bridges served IDs to models.dev's canonical metadata naming where they differ (e.g. `mistralai/Mistral-Medium-3.5-128B` → `mistral/mistral-medium-2604`). Canonical paths are provider-agnostic — often release-date slugs rather than the host's internal model name — so an alias does not mean the wrong model is referenced. Org-prefix mapping (`ORG_TO_MODEL_PROVIDER`) is case-insensitive, matching Hugging Face's case-insensitive org URLs.
