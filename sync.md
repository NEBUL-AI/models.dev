...
- Evidence rule for Nebul reasoning claims (relay host): never author a reasoning claim — an effort set, `[]`, or `reasoning = false` — from catalog absence alone. `supports_reasoning = false` can lag server-side enablement (the same flag is `false` for `openai/gpt-oss-120b`, whose docs show live reasoning with effort levels). Each claim needs dated per-ID evidence in the leading header: the catalog's advertised `reasoning_efforts` with a re-verification date, or a live chat-completions probe (per-effort acceptance plus trace-field observation with a date, as in the ai& provider's headers).
